...
def generateRand():
    itemId=''
    for i in range(0,3):
        randNo=random.range(0,(len(numeric)-1))
...
def save():
    ...
    if not(itemId and itemId.strip()) or not(name and name.strip()) or not(price and price.strip()) or not(qnt and qnt.strip()) or not(cat and cat.strip()):
        messagebox.showwarning("","Please Fill Everything")
        return
    if len(itemId) < 5:
    ...
    try:
        ...
        messagebox.showwarning("","Error In Saving")
        return
    refReshTable()
...
def update():
    ...
    try:
        ...
        messagebox.showwarning("","Please Select A Data")
    ...
    try:
        ...
        messagebox.showwarning("","Something Went Wrong"+str(err))
        return
...
def delete():
    try:
        if(my_tree.selection()[0]):
            decision = messagebox.askquestion("","Are You Sure You Want To Delete The Selected Data?")
            if(decision != 'yes'):
                return
            else:
                selectedItem = my_tree.selection()[0]
...
def find():
    ...
    if(itemId and itemId.strip()):
        ...
        messagebox.showwarning("","Please Fill At Least One Of The Entries")
        return
    cursor.execute(sql)
    try:
...
entiresFrame=tkinter.LabelFrame(frame,text="Form",borderwidth=5)
entiresFrame.grid(row=1,column=0,sticky="w",padx=[10,200],pady=[0,20],ipadx=[6])




itemIdLabel=Label(entiresFrame,text="ITEM ID",anchor="e",width=10)
nameLabel=Label(entiresFrame,text="NAME",anchor="e",width=10)
priceLabel=Label(entiresFrame,text="PRICE",anchor="e",width=10)
qntLabel=Label(entiresFrame,text="QNT",anchor="e",width=10)
categoryLabel=Label(entiresFrame,text="CATEGORY",anchor="e",width=10)
...
categoryArray = ['Food','Drink','Technology']




itemIdEntry=Entry(entiresFrame,width=50,textvariable=placeholderArray[0])
nameEntry=Entry(entiresFrame,width=50,textvariable=placeholderArray[1])
priceEntry=Entry(entiresFrame,width=50,textvariable=placeholderArray[2])
qntEntry=Entry(entiresFrame,width=50,textvariable=placeholderArray[3])
categoryCombo=ttk.Combobox(entiresFrame,width=47,textvariable=placeholderArray[4],values=categoryArray)
...
generateIdBtn = Button(entiresFrame,text="Generate ID",borderwidth=3,bg=btnColor,fg='white',command=generateRand)
